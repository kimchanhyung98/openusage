...
새 연동에 권장하는 라우트이며, `openusage` CLI와 같은 스키마 사용.
...
**활성화된** 모든 프로바이더의 레거시 UI 지향 스냅샷을 저장된 프로바이더 순서대로 반환.
...
대시보드의 **Single Card** / **Separate Cards** 모드와 Settings 계정 순서는 API 결과를 필터링하거나 재정렬하지 않음.
Single Card가 프로바이더의 계정 하나만 보여 줘도 사용 가능한 계정 스냅샷은 계속 조회 가능.
...
`GET` 이외의 메서드는 **405**, 알 수 없는 라우트는 **404**.
쿼리는 라우팅에 영향을 주지 않으며, 경로가 비어 있거나 쿼리만 있는 `GET` 요청은 **404** 반환.
쿼리에 지원 라우트 이름이 있어도 쿼리만 있는 요청은 알 수 없는 라우트로 로그 기록.
...
두 응답 형식 모두에서 Claude·Codex의 `displayName`은 `Claude`·`Codex` 같은 프로바이더명 유지.
Separate Cards 모드에서도 계정명이나 대시보드의 `{Provider}: {name}` 제목은 여기에 노출되지 않음.
신뢰된 1회 실행 CLI는 같은 스키마를 사용하지만 `displayName`에 해석된 Claude·Codex 계정명을 유지할 수 있음.
...
요청 수신·응답 송신 중 상대 연결 초기화나 broken pipe는 진단에서 취소된 요청으로 집계하며, 그 외 전송 오류는 실패로 유지.

## 브라우저 경계 및 개인정보

서버는 루프백 인터페이스(`127.0.0.1`)에서만 수신하고 `Host`를 `127.0.0.1[:6736]` 또는 `localhost[:6736]`로 제한.
DNS rebinding과 임의 웹 페이지의 loopback 읽기를 막기 위해 `Origin`이 있는 요청을 거부하고 CORS 응답 헤더를 제공하지 않음.
따라서 `curl`과 native local app의 `GET` 연동은 유지되지만 브라우저 페이지와 CORS preflight는 지원하지 않음.
응답에는 메뉴 막대와 같은 사용량 수치가 포함될 수 있으나 인증 정보나 토큰은 제공하지 않음.

## 캐싱 동작

API는 앱의 프로바이더 스냅샷을 조회 — 가져오기에 성공한 경우에만 데이터를 교체하므로 새로 고침 실패로 API가 비는 일은 없고, 마지막 정상 스냅샷을 계속 받음.
[새로 고침 및 캐싱](/docs/refreshing.md) 참조.
