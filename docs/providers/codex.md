...
| Reset Watch | 확률과 시각으로 표시하는 전역 재설정 예측 — 선택적이며 기본값은 꺼짐(아래 참조) |
...
전역·비공식 예측이며 OpenAI의 약속이나 보장이 아님.
API에 리셋 예정 정보가 있으면 예측 확률 **99% chance**와 Mac의 로컬 시간대 기준 **Scheduled {날짜} at {시각}** 표시.
99%는 해당 정보에 대한 OpenUsage의 고정 예측값이며 API에서 제공한 확률이 아님.
이 99% 예측에는 커뮤니티 투표율이 없으므로 조회·표시하지 않음.
API의 리셋 예정 정보에 시각이 없으면 예측 확률 99%와 **Reset time unknown** 표시.
예정 시각 경과는 실행 완료 증거가 아니며, 조회 성공으로 변경을 확인할 때까지 99% 예측 유지.
그 외에는 API의 활성 watch 확률과 기한으로 예측 표시.
예측 확률은 같은 활성 watch에 대한 커뮤니티 투표율(전체 응답 중 **Yes** 비율)과 별개로 유지.
...
리셋 예정 정보가 없고 활성 watch에 인식 가능한 출처 게시물이 있으면 `GET https://codex-resets.com/api/watch/votes`도 조회하며 해당 게시물과 일치하는 집계만 커뮤니티 투표율 계산에 사용.
...
푸터의 수동 Refresh(⌘R)도 예측·해당하는 커뮤니티 투표율을 사용량과 함께 조회 — 새 캐시도 재검증하되 재시도 대기는 유지하고 이미 진행 중인 요청은 공유.
...
예측 확률·시각·해당하는 커뮤니티 투표율은 계정별 값이 아닌 전역 정보.
...
커뮤니티 투표율 조회 실패·빈 집계·잘못된 값·게시물 불일치는 예측을 유지하면서 **Vote share unavailable** 표시.
커뮤니티 투표율 조회 실패 시 해당 조회 요청만 연기하며 사용량과 예측은 정상 새로 고침 유지.
...
예측 시각은 사용량 재설정 날짜와 같은 지역화된 `날짜 at 시각` 형식과 앱의 12/24시간 설정 사용.
해당 시각에 마우스를 올리면 사용량 초기화 시각과 같은 **Resets in …** 카운트다운 표시, 5분 이내이거나 시각이 지나면 **Resets soon** 표시.
행이 보이는 동안 30초마다 카운트다운 갱신; 시각 미상·조회 실패 상태에는 시각 툴팁 없음.
리셋 예정 정보가 없고 조회 성공 후 활성 watch가 없거나 확률이 없거나 예측 기한이 지나면 기한·커뮤니티 투표율 없이 **0% chance** 표시.
...
pi가 기록한 메시지별 양수 비용은 그대로 사용하고, 비용이 없거나 0이면 Codex 로그와 같은 요청별 가격 규칙으로 추정.
명시된 비용이 잘못된 값이면 재추정하지 않고 해당 행 제외.
pi의 일반 입력과 캐시 토큰은 이미 분리된 값으로 계산하며, 로그에 없는 fast/priority 설정을 현재 계정 설정에서 추측하지 않음.
pi가 Codex 사용량에 지원되지 않는 1시간 캐시 쓰기를 기록하면 모델명 유무와 관계없이 해당 요청을 추정 합계에서 제외하고 카드에 경고 표시.
추정 가능한 요청이 함께 있으면 해당 요청만 표시하며, 손상·미지원 요청만 있으면 마지막 정상 지출 이력 유지.
...
다음 정상 누적값은 마지막 정상 누적값과 비교하여 복구 — 동일 사용량의 중복 합산 방지.
부모 이력을 물려받은 자식 세션은 정상 재생 누적값이 확인된 뒤에만 누적 차이 집계.
재생 기록 자체가 없거나 누적값이 손상·누락된 경우에는 자식 사용량과 구분할 기준을 미확정으로 유지.
다음 정상 누적값으로 기준을 다시 잡고, 그 사이 개별 요청 값이 없는 증가분은 합산하지 않고 불완전 이력 경고 표시.
정상적인 개별 요청 값은 해당 요청만 집계하며, 누적 기준은 재생·실시간 구간에서 정상 누적값이 확인될 때 복구.
...
토큰 수 자체는 측정값.
자동 리뷰는 모델별 내역에서 `codex-auto-review`로 구분하고, 비용만 날짜별 참조 모델의 API 요율로 추정.
참조 모델은 실제 자동 리뷰 백엔드를 확인한 정보가 아니며, 참조 요율이 없으면 자동 리뷰 이름으로 미산정 경고 표시.
업데이트 후 남아 있는 로컬 로그를 다시 읽어 모델 구분을 복원하며, 원본 로그 없이 다른 Mac에서 동기화한 과거 합계는 임의로 재분류하지 않음.
하위 에이전트와 포크된 세션은 부모 세션의 토큰 이력을 자체 로그에 복사하지만, OpenUsage가 복사본을 인식해 하위 에이전트 수와 관계없이 각 토큰을 한 번만 집계.
로그 데이터는 Mac 밖으로 전송하지 않음.

GPT-5.4·GPT-5.5와 해당 Pro 모델, GPT-5.6 Sol·Terra·Luna, GPT-6 Astra는 캐시 읽기·쓰기를 포함한 입력 토큰이 272k를 넘는 요청 전체에 OpenAI 긴 컨텍스트 요율 적용.
프로바이더 접두사·날짜가 붙은 모델에도 같은 요청별 장문·priority 요율 적용.
Reasoning effort 별칭은 보충 가격표에 등록된 GPT-5.4·GPT-5.5 일반 모델, GPT-5.6 Sol·Terra·Luna, GPT-6 Astra의 등급만 지원; Pro 모델의 effort 접미사는 해당 지원 범위에서 제외.
접두사 또는 날짜가 포함된 이름의 명시 요율은 별칭보다 먼저 조회하며, 없으면 보충 가격표와 접두사 없는 이름으로 재조회.
Fast 별칭은 등급·날짜를 유지한 같은 프로바이더의 기본 요율을 먼저 확인한 뒤 배율을 한 번 적용.
Codex 모델명의 날짜(`-YYYYMMDD`·`-YYYY-MM-DD`) 바로 앞 `.fast`·`@fast`도 `-fast`와 같은 요청 요율로 계산.
날짜가 붙은 이름으로 요금을 찾지 못하면 `-YYYYMMDD`·`-YYYY-MM-DD` 접미사를 제거해 보충 가격표와 별칭 재조회.
기본 단가의 입력·캐시 2배, 출력 1.5배 적용; 정확히 272k인 요청은 기본 단가 유지.
가격 출처에 캐시 읽기 할인이 있으면 캐시 입력에 해당 할인 적용, 없으면 전체 입력 요율로 추정.
Fast/priority 추정에는 모델별 공개 Codex 배율 적용(예: GPT-5.5는 2.5×); `-fast`·`.fast`·`@fast`로 끝나는 모델 이름은 배율을 한 번 적용하기 전에 배율 적용 전 기본 요율로 정규화.
...
**Use**에서 저장 로그인의 키체인 승인이 필요하면 화면 응답을 유지한 채 승인 대기.
저장 로그인 읽기에 실패하면 크레딧 사용 요청 없이 실패 안내.
**Use** 이후 자동 재조회는 추가 승인창 없이 진행하며, 승인 필요로 실패하면 수동 새로 고침으로 재시도.
...
사용 속도 예측은 실제 사용량이 있고 유의미한 계산이 가능할 만큼 기간이 지난 뒤에만 시작.
