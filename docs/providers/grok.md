...
달러 금액은 공유 [모델 가격](/docs/pricing.md)을 사용해 공개 API 요율과 토큰 수로 추정한 값(ⓘ 표시)이며, 토큰 수 자체는 측정값이고 이 추정치는 결제 API가 반환하는 월간 크레딧과 별개.
로그 데이터는 Mac 밖으로 전송되지 않음.
음수·소수·표현 범위 초과 등 잘못된 토큰 수나 프로세스 ID가 있는 사용량 행은 제외하고, [불완전한 로컬 이력 안내](/docs/refreshing.md#가져오기에-실패한-경우) 표시.
사용량을 담지 않는 모델 변경 기록의 손상된 프로세스 ID는 숫자 경고에 포함하지 않으며, 실제 사용량 행의 ID는 별도로 검증.
정상적인 과거 모델 변경은 계속 사용하여 기간 경계에 걸친 요청의 모델 식별.
