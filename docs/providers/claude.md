...
사용 가능한 계정별 토큰을 이전 형식의 토큰보다 우선 선택 — 서로 다른 권한 범위의 항목에도 적용.
현재 계정 소유가 확인된 항목을 소유자 정보 없는 이전 형식 항목보다 우선하며, V1·V2 버전이 달라도 동일하게 적용.
소유권 조건이 같으면 V2 우선이며, V2의 삭제 마커는 V1 사본도 차단.
현재 계정의 항목이 삭제·만료·손상된 경우 같은 항목의 소유자 미확인 토큰으로 되돌리지 않음.
계정 UUID 필드가 없는 구형 설정은 이전 형식의 유효한 로그인만 확인.
계정 UUID가 손상되었거나 현재 조직을 확인할 수 없으면 Desktop 인증 실패 처리.
계정별 캐시 항목의 소유자 UUID가 손상되었거나 구분자가 없으면 오류 기록 후 Desktop 인증 실패 처리하며, 이전 형식 토큰으로 대체하지 않음.
소유자 UUID가 정상인 다른 계정 항목은 선택 대상에서만 제외.
같은 캐시 항목의 별칭이 충돌하면 삭제 마커 우선, 나머지는 정상 토큰 중 만료가 늦은 항목 선택.
...
달러 금액은 토큰 수에 API 요율과 공유 [모델 가격](/docs/pricing.md)을 적용한 추정치(ⓘ의 의미)이며 토큰 수 자체는 측정값.
...
커스텀 설정 디렉터리(별도 `CLAUDE_CONFIG_DIR` 홈)에 있는 Claude 로그인은 카드도, 계정 선택기 항목도 아니며 [CLI](/docs/cli.md)·[로컬 API](/docs/local-http-api.md)에도 없음.
...
비활성 계정 카드는 [로컬 API](/docs/local-http-api.md)에 `claude@profile-…` 같은 ID로 표시.
...
**Single Card**에서 비활성 스냅샷 계정을 볼 때 해당 행에 기존처럼 **No data** 표시 가능.
...
Claude가 활성화돼 있으면 실행 시, 활성화 시, 5분마다, 대시보드 수동 새로 고침 시 [Claude Status](https://status.claude.com/)의 Claude API와 Claude Code 컴포넌트 확인.
...
5시간 세션 사용량이 0%이고 초기화 시각도 없으면 Session 행의 후행 레이블에 **Not started** 표시; 마우스를 올리면 첫 메시지 후 세션이 시작된다는 설명 제공.
초기화 시각이 있으면 1% 미만 사용량이 0%로 보고돼도 시작된 기간으로 표시하며, 카운트다운과 초기화 시각 표시 전환 유지.
