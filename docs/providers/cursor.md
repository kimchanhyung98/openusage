...
Cursor가 활성화돼 있으면 실행 시, 활성화 시, 5분마다, 대시보드 수동 새로 고침 시 [Cursor Status](https://status.cursor.com/)의 IDE와 cursor.com 컴포넌트 확인.
...
일별 지출 추정에는 공유 [모델 가격](/docs/pricing.md)을 적용한 내보내기 토큰 수 사용; Cursor 전용 모델(`auto`, `composer-*`, …)은 유지관리자가 [Cursor 모델 및 가격](https://cursor.com/docs/models-and-pricing.md)에서 동기화하는 보충 가격 정보에서 가져옴.
