...
스토어가 `refresh()` 대기를 시스템 잠자기를 제외한 2분으로 제한하고 시간 초과·앱 종료 시 작업 취소 요청.
취소에 협조하지 않으면 그 작업이 실제로 끝날 때까지 같은 프로바이더의 다음 조회 보류 — 긴 루프에서 `Task.isCancelled` 확인, 네트워크 호출은 취소 가능한 형태로 유지.
자세한 동작은 [새로 고침](/docs/refreshing.md) 참고.
