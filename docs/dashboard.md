...
- 채움 색은 현재 소진 속도를 근거로 기간 전체를 판정한 결과: 최소 10%를 남기고 끝낼 궤도면 파랑, 마지막 10% 안쪽에 들어가지만 약간의 여유는 남을 전망이면 노랑, 초기화 전에 다 쓰거나 여유 없이 한도에 딱 맞춰 끝날 전망이면 빨강.
  ...
  아직 사용량이 없는 기간, 초기화 기간이 없는 막대(크레딧 잔액 등), 예측하기엔 너무 이른 새 기간은 대신 사용량 수준 자체로 색 결정: 80%를 쓰면 노랑, 10% 이하가 남으면 빨강.
...
- **Soft Limit**을 켜면 지원 대상 5시간 또는 주간 막대에 직접 입력한 1%–100% 임계값의 얇은 안내선 표시 — 저장된 임계값이 없으면 기본값 90%.
  프로바이더 hard limit이 아닌 사용자의 조기 한도선이며, 채움·사용량·속도 판정·초기화에는 영향 없음.
  Used와 Left에서 위치만 반전 — 95% used 임계값은 반대 화면의 5% left 위치.
  도달 문구는 사용률을 정수 퍼센트로 반올림한 값 기준 — 94.6%도 95% 안내선 도달로 판정.
  달러 기반 막대에도 같은 사용률 기준 적용.
  Left 전환 시에도 사용량 기준 비교 유지.
  안내선 도달은 막대 hover 문구에 표시되며 취소 완료 증거는 아님.
  취소 엔진은 반올림 전 fresh 사용량 사용 — Codex를 포함한 클라이언트·계정 범위 검증 전 자동 취소 차단.
  활성화·연결·취소 상태는 [Soft Limit 설정](/docs/settings.md#soft-limit) 참조.
...
- 파란 막대는 기본적으로 추가 표시 없음.
  **Always Show Pacing**(사용 속도 항상 표시)을 켜면(설정) 막대의 균등 속도 눈금과 지표 이름 옆 작은 `~35% left at reset` 메모까지 표시.
  사용량이 0이면 이 설정을 켜도 예측 문구와 균등 속도 눈금 미표시.
...
**Codex Reset Watch**는 할당량 동작 없이 같은 막대 형태로 전역 리셋 예측 표시.
API에 리셋 예정 정보가 있으면 예측 확률 **99% chance**와 `Scheduled {날짜} at {시각}` 표시; 99%는 API 확률이 아닌 OpenUsage의 고정 예측값.
리셋 예정 정보에 시각이 없으면 **Reset time unknown** 표시; 예정 시각이 지나도 리셋 완료로 처리하지 않음.
그 외에는 API의 활성 watch 확률과 고정 `By …` 기한으로 예측 표시.
예측은 Used/Left, 리셋 형식, 속도, 할당량 알림 설정의 영향을 받지 않음.
활성 watch로 예측할 때는 커뮤니티 투표율(전체 응답 중 **Yes** 비율)을 예측 확률과 별도로 표시, 집계가 없거나 조회 실패 시 예측을 유지하면서 **Vote share unavailable** 표시.
예측 시각은 사용량 재설정 날짜와 같은 `날짜 at 시각` 형식과 12/24시간 설정 사용.
마우스를 올리면 사용량 초기화 시각과 같은 **Resets in …** 또는 **Resets soon** 툴팁 표시; 시각 미상·조회 실패 상태에는 시각 툴팁 없음.
막대는 40% 미만에서 중립 상태, 40–59%에서 파랑, 60–69%에서 노란 경고 삼각형, 70% 이상에서 빨간 불꽃 표시.
확률이 높을수록 예상 재설정 전에 남은 토큰을 먼저 쓰라는 경고 강화.
조회 실패 시 시각 대신 사용 불가 또는 캐시 예측 안내 표시.
리셋 예정 정보가 없고 조회 성공 후 활성 신호가 없거나 확률이 없거나 예측이 만료되면 기한·커뮤니티 투표율 없이 **0% chance** 표시, 첫 조회 전에는 **No data** 표시.
예측 만료 시 행 타이머 중단.
...
모델별 비용 내역은 센트 단위로 반올림하되, 소수부를 표현할 수 없는 큰 유한 금액은 원래 값 유지.
표시 반올림 때문에 비용이 무한대로 바뀌거나 스냅샷 저장이 실패하지 않도록 처리.
...
초기화 시각이나 알려진 Reset Watch 시각이 있는 행은 30초마다 다시 렌더링되어 네트워크 새로 고침 사이에도 카운트다운·시각 툴팁·속도·예측 만료가 최신 상태 유지.
이 UI 갱신은 네트워크 요청을 만들지 않으며, Reset Watch 소스 조회는 별도 15분 주기로 실행.
수동 Refresh(⌘R)도 활성 Reset Watch의 예측·해당하는 커뮤니티 투표율을 사용량과 함께 재검증하되 오류 후 재시도 대기는 유지.
