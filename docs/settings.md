...
| Always Show Pacing(사용 속도 항상 표시) | 끄기 / 켜기 | 켜기(기본값)는 초기화 기간이 있는 모든 지표에 사용 속도 표시 — 정상 궤도인 행에는 예측치("~33% left at reset")와, 꾸준히 썼다면 지금 어디쯤일지 알려 주는 균등 속도 눈금 추가.<br>끄면 한도에 가깝거나 넘긴 지표로만 제한.<br>사용량이 0이거나 초기화 기간이 없는 지표에는 예측 문구·눈금 미표시. |
