...
| 설정 | 옵션 | 동작 |
|---|---|---|
| Enabled(활성화) | 끄기 / 켜기 | 기본값 끄기, 활성화 전 확인 필요.<br>지원 대상 진행 막대에 안내선 표시.<br>클라이언트·계정 범위 검증 전 자동 취소 차단. |
| Window(기간) | 5 Hours / Weekly | 한 번에 하나의 soft-limit 기간 선택.<br>기본값 Weekly, Soft Limit을 켠 동안에만 표시.<br>일간·월간·잔액·상한 없는 지표 제외. |
| Threshold(임계값) | 정수 직접 입력, 1%–100% | 안내선과 지원 대상 자동 취소의 사용률 임계값 직접 입력.<br>저장된 임계값이 없으면 기본값 90%, Soft Limit을 켠 동안에만 표시.<br>프로바이더 hard limit이 아닌 사용자 기준선. |
