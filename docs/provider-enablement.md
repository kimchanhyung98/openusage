...
이미 등록한 Claude 계정의 저장 로그인은 Keychain 확인이 일시적으로 불가능해도 탐지 대상 유지.
등록 계정의 카드도 확인 불가만으로 숨기지 않으며, 실제 사용량 조회에서 인증 접근 실패 안내.
미등록 로그인은 확인된 인증 흔적이 있어야 자동 탐지 — 접근 문제로 누락됐다면 Customize에서 직접 켜기 가능.
