...
Keychain 비밀번호 저장은 macOS Security API를 사용하며 비밀 값을 외부 명령의 인자로 전달하지 않음.
기존 항목은 읽기와 같은 키체인 검색 목록에서 찾아 갱신하고, 항목이 없을 때만 기본 키체인에 생성.
계정 이름을 지정한 저장은 검색 목록에서 처음 일치하는 항목만 갱신.
...
각 스냅샷에는 해당 계정명에 현재 저장된 인증 파일 내용이 담기고, 이 Mac의 키체인에만 비공개로 남으며, 어디에도 전송되지 않음.
