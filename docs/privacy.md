...
앱 전용 Keychain 비밀번호 저장은 macOS Security API 사용.
계정 인증 스냅샷은 조회에도 같은 API를 사용하므로 토큰 갱신 후 별도 시스템 도구의 읽기 승인 불필요.
자동 갱신·계정 준비 상태 확인은 승인창 없이 조회하며, 접근 불가 시 오류 기록.
이전 버전에서 저장한 항목은 수동 새로 고침·계정 전환·재로그인 시 OpenUsage의 접근 승인을 한 번 요청할 수 있음.
