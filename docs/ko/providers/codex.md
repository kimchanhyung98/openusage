...
계정을 관리하고 공유 `auth.json`이 존재하는 동안 해당 파일은 **Default**(공유 홈) 카드 전용 인증 정보 출처.
...
Separate Cards 제목은 **Codex: sub** 같은 고정 형식 **{Provider}: {name}** 사용.
계정 순서는 [설정](/docs/ko/settings.md), 카드 표시·공유 동작은 [대시보드](/docs/ko/dashboard.md) 참조.

대시보드 계정 선택기에는 공유 홈 계정과 이 Mac에 인증 스냅샷이 저장된 등록 계정 표시.
