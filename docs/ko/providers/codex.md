...
AI 확률은 같은 활성 watch에 대한 커뮤니티 **Yes** 투표 비율과 별개로 유지.
...
인식 가능한 출처 게시물이 있는 활성 예측은 `GET https://codex-resets.com/api/watch/votes`도 조회하며 해당 게시물과 일치하는 투표만 사용.
투표 엔드포인트는 웹사이트 내부용이며 문서화된 공개 API에 포함되지 않아 호환성 변경 가능.
OpenUsage는 집계만 조회하며 투표를 제출하지 않음.
지표를 활성화하거나 활성 상태에서 Codex 프로바이더를 다시 켜면 즉시 조회.
푸터의 수동 Refresh(⌘R)도 활성 예측과 커뮤니티 투표를 사용량과 함께 조회 — 새 캐시도 재검증하되 재시도 대기는 유지하고 이미 진행 중인 요청은 공유.
...
커뮤니티 투표 비율도 공유하며 웹사이트의 실시간 주기가 아닌 동일 조회 시점에 갱신.
투표 조회 실패·빈 집계·잘못된 값·게시물 불일치는 AI 예측을 유지하면서 **Votes unavailable** 표시.
투표 실패 시 투표 요청만 연기하며 사용량과 AI 예측은 정상 새로 고침 유지.
투표 엔드포인트의 `Retry-After` 준수, 유효한 대기 시간이 없으면 요청 제한 후 5분·그 외 실패 후 1분 대기.
별도 타이머 없이 대기 시간이 지난 다음 수동 또는 자동 조회에서 투표 재시도.
