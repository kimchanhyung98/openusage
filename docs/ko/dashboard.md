...
Claude, Codex, Kimi로 시작한 뒤 Mac에 인증 정보가 있는 프로바이더(기존 로컬 로그인, 저장된 API 키, 지원하는 환경 변수 — 어디에도 전송하지 않음)를 빠르게 찾아 해당 조합만 활성화.
아무것도 찾지 못하면 Claude/Codex/Kimi 시작 세트 유지.
...
[**Settings → Accounts**(설정 → 계정)](/docs/ko/settings.md)에 Claude 또는 Codex 계정이 둘 이상 등록되면, 관리형 사용량 보기가 **Claude** 또는 **Codex** 카드 하나를 공유.
카드 헤더의 작은 계정 선택기는 등록된 계정만 나열하고, 카드에 어느 계정의 사용량을 표시할지 선택.
선택기는 보기 전용이며, 로그인이나 로그아웃을 실행하지도, 새 터미널 세션이 사용할 계정을 바꾸지도 않음.
터미널 계정 전환은 Settings에서만 수행.
Settings에서 전환을 확정하면 이 선택기와 해당 프로바이더의 메뉴 막대 별표를 같은 계정으로 한 번 이동.
비활성 계정의 사용량은 비공개 Keychain 인증 스냅샷에서 읽음.
Settings에서 계정을 추가·이름 변경·재로그인·제거하면 선택기 즉시 갱신.
커스텀 설정 디렉터리에서 독립적으로 찾은 Claude 계정은 기존의 별도 카드와 이름 유지.
별도로 발견한 관련 없는 카드는 관리형 선택기 안에 숨기지 않음.
자동 탐색 카드는 프로바이더 신원으로 중복 제거하지만, 관리형 레코드는 사용자가 붙인 계정명으로 구분.
따라서 두 관리형 계정명의 저장 인증 정보가 현재 같은 프로바이더 신원을 증명해도 선택기에는 별도 항목으로 유지.
관리형 선택기에 이미 있는 신원의 자동 탐색 카드는 표시하지 않고, 없는 신원은 자동 탐색 카드 하나로 중복 제거.
로컬 지출·추이 로그는 관리형 계정에 귀속되지 않고 해당 설정 홈에 그대로 남음.
...
Hide는 프로바이더 전체를 끄고, Customize는 다시 켜거나 해당 프로바이더의 지표 화면으로 이동.
자체 계정 레코드를 가진 Claude 또는 Codex 카드 — 계정이 한 번 관찰된 기본 카드 포함 — 는 **Rename…**(이름 바꾸기)도 제공 가능.
관리형 계정명은 별개이며 **Settings → Accounts → Manage…**에서 편집.
대시보드 카드 제목은 **Claude** 또는 **Codex**로 유지되고, 선택기에 관리형 계정명 표시.

## 공유

한 프로바이더의 사용량을 깔끔한 브랜드 PNG로 클립보드에 복사해 채팅, 트윗, 문서에 바로 붙여넣기 가능.
방법은 두 가지:

- 프로바이더 헤더를 우클릭하고 **Share Screenshot** 선택.
- 푸터의 **Options**(옵션) 메뉴에서 **Share Screenshot** ▸ *\<프로바이더\>* 선택.
  하위 메뉴에는 현재 대시보드에 보이는 모든 프로바이더 나열.

이미지는 앱 스타일을 따르는 가변 높이 PNG — 상단에 프로바이더 마크와 이름, 중간에 현재 표시 중인 지표 행, 하단 중앙에 작은 OpenUsage 마크 배치.
...
오른쪽에는 **Options** 메뉴 버튼.
여기에 모든 것이 모여 있음 — **Customize**, **Settings**, **Share Screenshot**(프로바이더 하위 메뉴), **Check for Updates…**, **About OpenUsage**, **Quit OpenUsage**.
...
**Name** 필드는 표시 중인 프로바이더 ID에 자체 계정 레코드가 있을 때만 표시 — 계정을 한 번이라도 확인한 기본 Claude/Codex 카드 포함.
이 필드는 해당 카드 레코드의 이름을 바꾸며, Settings의 관리형 계정명과는 별개.
...
드래그 순서 변경은 대시보드에서도 바로 동작 — 같은 프로바이더 안에서 행을 드래그하거나, 카드가 열린 상태에서 캐럿 경계를 넘겨 드래그하거나, 프로바이더 헤더를 드래그해 섹션 순서 변경.
