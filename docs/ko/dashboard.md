...
- **Soft Limit**을 켜면 지원 대상 5시간 또는 주간 막대에 설정한 90%–95% 임계값의 얇은 안내선 표시.
  프로바이더 hard limit이 아닌 사용자의 조기 한도선이며, 채움·사용량·속도 판정·초기화에는 영향 없음.
  Used와 Left에서 위치만 반전 — 95% used 임계값은 반대 화면의 5% left 위치.
  도달 문구는 표시 정밀도로 반올림한 사용량 기준 — 95%로 표시되는 94.6%도 95% 안내선 도달로 판정.
  Left 전환 시에도 사용량 기준 비교 유지.
  안내선을 넘으면 막대 hover 문구에 표시하지만, 현재 버전은 관련 App·CLI 세션 전체를 정확하게 제어할 수 있음을 아직 증명하지 못해 AI 세션 자동 중단 비활성.
...
Claude와 Codex는 계정 수와 무관하게 **Claude** 또는 **Codex** 제목의 카드 하나만 표시.
계정이 둘 이상이면 카드 헤더의 작은 계정 선택기가 어느 계정의 사용량을 표시할지 선택.
선택기에는 프로바이더 공유 홈(`~/.claude`, `~/.codex`)에 로그인된 계정을 **Default**로, [**Settings → Accounts**(설정 → 계정)](/docs/ko/settings.md)에 등록한 계정 중 이 Mac에 쓸 수 있는 저장 인증이 있는 것을 각 계정명으로 표시.
저장 인증이 없는 등록 계정은 표시할 사용량이 없으므로, 다시 로그인하기 전까지 선택기에서 제외.
...
카드 제목은 고정 — Claude·Codex 카드는 카드 헤더, 이 메뉴 항목, 공유 스크린샷, 메뉴 막대 모두에서 항상 프로바이더 이름 사용.
계정명은 **Settings → Accounts → Manage…**에서만 편집.
