...
계정 표시와 공유 레이아웃 동작은 [대시보드](/docs/ko/dashboard.md) 참조.
...
**Ready**와 **Sign-In Needed** 행 모두 이동 가능하며 Claude와 Codex 사이의 이동은 허용하지 않음.
...
계정 교체 시 이전 계정의 진행 중인 할당량 알림은 중단하거나 제출된 요청 제거.
...
| Share Anonymous Usage(익명 사용량 공유) | 켜기 / 끄기 | 기본값은 끄기.<br>계정 정보·인증 정보·사용량 값 없이 일일 요약·제한된 실패/복구 진단·조건에 맞는 크래시 리포트 공유.<br>필드와 동의 동작은 [개인정보 및 사용 데이터](/docs/ko/privacy.md) 참조. |
