...
- Customize에는 Claude 또는 Codex가 한 번만 나오고, 켜기/끄기는 해당 프로바이더 계열의 모든 계정 카드에 적용.
  **Single Card** 모드의 대시보드 계정 선택기는 공유 홈의 계정과 여기 등록한 계정을 나열하고, 카드에 어느 계정의 사용량을 표시할지 선택.
...
| **Separate Cards** | 사용 가능한 계정 카드를 `Claude: company`, `Codex: sub` 같은 고정 제목 `{Provider}: {name}`으로 각각 표시.<br>헤더 계정 선택기 없음. |
