...
- Without a PR number: `gh api /repos/kimchanhyung98/openusage/commits/{full_hash} -q '.author.login'`.
...
curl -s "https://openusage.chanhyung.kim/appcast.xml" | grep -F "OpenUsage-{version}.dmg"
```
...
if [ "$(gh release view "$tag" --json isDraft --jq '.isDraft')" = "false" ]; then
  gh api repos/kimchanhyung98/openusage/releases --paginate \
    --jq '.[] | select(.draft and .tag_name=="'"$tag"'") | .id' \
    | xargs -I{} gh api -X DELETE repos/kimchanhyung98/openusage/releases/{}
else
...
### New Features
- {message} ([#{pr}](https://github.com/kimchanhyung98/openusage/pull/{pr})) by @{author}
